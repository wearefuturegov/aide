  ...
  bot.dialog('equipmentType',
    new builder.IntentDialog({ recognizers: [recognizer] })
      .matches('equipment.bathroom', function(session) {
        session.send(config.equipment.bathroom.message);
        session.send(config.equipment.bathroom.link);
      })
      .matches('equipment.bedroom', function(session) {
        session.send(config.equipment.bedroom.message);
        session.send(config.equipment.bedroom.link);
      })
      .matches('equipment.diningroom', function(session) {
        session.send(config.equipment.diningroom.message);
        session.send(config.equipment.diningroom.link);
      })
      .matches('equipment.garden', function(session) {
        session.send(config.equipment.garden.message);
        session.send(config.equipment.garden.link);
      })
      .matches('equipment.office', function(session) {
        session.send(config.equipment.office.message);
        session.send(config.equipment.office.link);
      })
      .matches('equipment.kitchen', function(session) {
        session.send(config.equipment.kitchen.message);
        session.send(config.equipment.kitchen.link);
      })
      .matches('equipment.lounge', function(session) {
        session.send(config.equipment.lounge.message);
        session.send(config.equipment.lounge.link);
      })
      .matches('equipment.stairs', function(session) {
        session.send(config.equipment.stairs.message);
        session.send(config.equipment.stairs.link);
      })
      .onDefault(function(session) {
